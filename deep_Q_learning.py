...
class deep_q_learning_algorithm(object):
    def __init__(self, episode, epsilon):
    ...
    def policy(self, state, action_size):
        if np.random.random() > self.epsilon:
            return np.argmax(deep_q_learning.get_q_value(state))
        else:
    ...
    def deep_q_learning(self):
        ...
        for episode in tqdm(range(1, self.episode+1), unit="episode"):
            ...
            step = 1
            state = mountaincar3d.start()
            reached_goal = False
            episode_reward = 0

            while not reached_goal and step <= 500:
                action = self.policy(state, action_size)
                action, reward, next_state, reached_goal = mountaincar3d.step(action)
                episode_reward += reward
                deep_q_learning.update_replay_memory((state, action, reward, next_state, reached_goal))
